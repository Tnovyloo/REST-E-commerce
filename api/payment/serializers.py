...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = ('id', 'buyer', 'status', 'payment_option', 'order', 'created_at', 'updated_at')
...
class CheckoutSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Shipping address for and order is not set
        if not instance.shipping_address:
            ...
            address = Address.objects.filter(shipping_address=instance.id)
